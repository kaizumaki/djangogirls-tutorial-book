...
正確には、ビューが モデルとテンプレートの橋渡しをしてくれます。 私達が作業している @<tt>{post_list} ビューの場合、表示したいデータを取り出して、テンプレートファイルに渡すことになります。 どのモデルのデータを、どのテンプレートに表示させるかを、ビューに記述します。
...
@<tt>{models} の前にあるドットは @<b>{カレントディレクトリ} 、もしくは @<b>{カレントアプリケーション} のことです。 @<tt>{views.py}と @<tt>{models.py}は、同じディレクトリに置いてあります。 だから、こんな風に@<tt>{.}とファイル名だけを使って、簡単に記述することが出来るのです。（ファイル名の拡張子@<tt>{.py}は必要ないです） そして、モデルの名前を指定してインポートします(この場合のモデルは @<tt>{Post}ですね)。
